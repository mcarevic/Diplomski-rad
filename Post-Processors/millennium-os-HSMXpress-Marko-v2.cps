...
function onCircular(clockwise, cx, cy, cz, x, y, z, f) {
  ...
  // Optional low memory mode: linearize all arcs
  if (getProperty("lowMemoryMode")) {
    writeln("");
  ...
  // RRF 3.3+ : Use plane codes and I/J/K conventions as usual, but for MOS + HSMXpress
  // we will always output arcs using radius (R) except for full circles.
  outputPlaneCommand(plane);

  // Full-circle arcs: use center format
  if (isFullCircle()) {
    if (isHelical()) {
      writeln("");
      ...
      // Non-helical full circle: use center-based arc output.
      outputFullCircleCommand(plane, clockwise, cx, cy, cz, f);
    }
    return;
  }

  // Non-full-circle arcs: always use radius so controller always sees R parameter.
