 ...
 */
// --- HSMXpress/HSMWorks compatibility shims ---

// Capability constants (stubbed where missing)
if (typeof CAPABILITY_MILLING == "undefined") {
    var CAPABILITY_MILLING = 0;
}

// Format type constants (stubbed)
if (typeof FORMAT_INTEGER == "undefined") {
    var FORMAT_INTEGER = 0;
}
if (typeof FORMAT_REAL == "undefined") {
    var FORMAT_REAL = 1;
}
if (typeof FORMAT_TZS == "undefined") {
    var FORMAT_TZS = 2;
}

// Variable control constants (stubbed)
if (typeof CONTROL_FORCE == "undefined") {
    var CONTROL_FORCE = 0;
}
if (typeof CONTROL_NONZERO == "undefined") {
    var CONTROL_NONZERO = 0;
}

// createOutputVariable fallback
if (typeof createOutputVariable == "undefined") {
    function createOutputVariable(config, format) {
        return createVariable(config, format);
    }
}

// getProperty fallback
if (typeof getProperty == "undefined") {
...
var rVar = createOutputVariable({ prefix: "R" }, radiusFmt);
...
function onOpen() {
  ...
  if(properties.versionCheck) {
    ...
    writeBlock(mCodes.format(M.VERSION_CHECK), 'V"v0.5.0",');
    writeln("");
  }
  ...
  if (properties.outputTools && nTools > 0) {
    ...
    for(var i = 0; i < nTools; i++) {
      ...
      // If description is empty (common in HSMXpress), generate a default one.
      var desc = tool.description;
      if (!desc || desc.length < 1) {
        desc = "Tool " + intFmt.format(tool.number);
      }
...
function outputFullCircleCommand(plane, clockwise, cx, cy, cz, f) {
  // Force linearization for full-circle arcs to avoid G2/G3 without IJK/R issues.
  writeln("");
  writeComment("Linearized full-circle arc (outputFullCircleCommand)");
  linearize(tolerance);
}
...
function outputArcCommand(plane, clockwise, cx, cy, cz, x, y, z, f) {
  // Force linearization for partial arcs to avoid G2/G3 without IJK/R issues.
  writeln("");
  writeComment("Linearized partial arc (outputArcCommand)");
  linearize(tolerance);
}


function outputArcRadiusCommand(clockwise, x, y, z, f) {
  // Force linearization for radius-based arcs to avoid G2/G3 without IJK/R issues.
  writeln("");
  writeComment("Linearized radius-based arc (outputArcRadiusCommand)");
  linearize(tolerance);
}
...
function onCircular(clockwise, cx, cy, cz, x, y, z, f) {
  // To avoid invalid G2/G3 arcs without I/J/K/R on some controllers,
  // always linearize circular moves into G1 segments.
  writeln("");
  writeComment("Linearized circular move (onCircular forced to lines)");
  linearize(tolerance);
}
