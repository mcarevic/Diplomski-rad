...
var iVar = createOutputVariable({ prefix: "I" }, axesFmt);
var jVar = createOutputVariable({ prefix: "J" }, axesFmt);
var kVar = createOutputVariable({ prefix: "K" }, axesFmt);
...
function outputArcCommand(plane, clockwise, cx, cy, cz, x, y, z, f) {
  ...
  var start = getCurrentPosition();
  var arcVars = getArcVars(plane, start, cx, cy, cz);
  var b1 = arcVars[0];
  var b2 = arcVars[1];
...
function onCircular(clockwise, cx, cy, cz, x, y, z, f) {
  ...
  if(plane === -1) {
    ...
    // Guard against degenerate tiny-radius arcs that confuse MOS/RRF:
    // if radius is effectively zero, linearize instead of outputting G2/G3 without IJK/R.
    var _r = getCircularRadius();
    if (Math.abs(_r) < 1e-6) {
      writeln("");
      writeComment("Linearized degenerate arc (radius too small)");
      linearize(tolerance);
      return;
    }
